...
class OurNeuralNetwork:
    ...
    *** ВАЖНО ***:
    ...
    def train(self, data, all_y_trues):
        learn_rate = 0.1 # Коэффициент скорости обучения
        epochs = 1000  # Число итераций обучения
        ...
        for epoch in range(epochs):
            ...
            if epoch % 10 == 0:
                ...
                print("Количество проходов %d Потеря: %.3f" % (epoch, loss))
...
aurum = np.array([-7, -3])  # 128 фунтов, 63 дюйма
silver = np.array([20, 2])  # 155 фунтов, 68 дюймов
print("Степень десорбции золота: %.3f" % network.feedforward(aurum))
print("Степень десорбции серебра: %.3f" % network.feedforward(silver))